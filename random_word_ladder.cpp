...
// class adjacency_node_s adjacency_node_t;
// class hash_table_node_s hash_table_node_t;
// class hash_table_s hash_table_t;

class adjacency_node_t
{
  adjacency_node_t *next;    // link to th enext adjacency list node
  hash_table_node_t *vertex; // the other vertex
};

class hash_table_node_t
{
  // the hash table data
...
int main(int argc, char **argv)
{
  ...
  for (i = 0u; i < hash_table->hash_table_size; i++)
    for (node = hash_table->heads[i]; node != nullptr; node = node->next)
